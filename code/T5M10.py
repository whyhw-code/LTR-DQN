...
if __name__ == '__main__':
    ...
    temp_data = pd.read_csv(f'temp/oc/batch{test_batch}/{bankuaicode}temp_test_{LTR}_train{train_year}_0.0003_0.001_0.001_5_1000.csv',
                            usecols=['qid_date', 'stock_code', 'real_return', 'prediction', 'close', 'pclose'])
    df = pd.read_csv('temp/oc/batch123/meiri_xuanze2.csv', usecols=['qid_date', '3068', '60'])
    ...
    while not done:
        ...
        try:
            action = df.loc[df['qid_date'] == env.qid_date, '60'].iloc[0]
        except:
            action = action1
