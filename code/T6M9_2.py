...
seed_path = os.path.join(current_dir, "temp", "seed_summary.csv")
...
# 自动获取当前脚本名，例如 T6C5_2
current_name = os.path.basename(__file__).replace(".py", "")
seed_list = seed_df.loc[current_name].dropna().tolist()

random_result = []

for random_zhongzi in seed_list:
    random_zhongzi = int(random_zhongzi)
...
mean_ = random_df[['ARR','dqn_ARR']].mean()

std_ = random_df[['ARR','dqn_ARR']].std()
std_ = std_.rename({'dqn_ARR': 'dqn_std'})
