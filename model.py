"""Models and shared implementation for the aligned reproduction."""
...
# Keep openpyxl's XML serializer stable even when lxml is installed globally.
...
def dqn_ranking_path(run_dir: Path, market: str, year: int, split: str) -> Path:
    """Return the LambdaRank ranking generated by the current training run.
    ...
    if not fresh.is_file():
        raise FileNotFoundError(
            f"Fresh LambdaRank {split} ranking not found: {fresh}. "
...
# T4M11/T4C11 are the repository's published T4 LambdaMART entry points.
# Their defaults are a calibrated source configuration and differ from the
# generic 5/1000 and 6/1000 configurations used by T5M9/T5C9.
T4_MART_HYPERPARAMETERS = {
    "0060": {"learning_rate": 0.001, "max_depth": 8, "n_estimators": 1000},
    "3068": {"learning_rate": 0.1, "max_depth": 4, "n_estimators": 1100},
}
...
def validate_runtime() -> None:
    ...
    mismatches = {
        name: {"expected": version, "actual": actual[name]}
        for name, version in expected.items()
        if actual[name] != version
    }
    if mismatches:
        details = ", ".join(
            f"{name}=={values['expected']} (found {values['actual']})"
...
def fit_ranker(
    ...
    if model_name == "LambdaRank":
        ...
        mart_params = (
            T4_MART_HYPERPARAMETERS[code]
            if int(train_year) == 3
            else PAPER_HYPERPARAMETERS["LambdaMART"][code]
        )
        params.update({
...
def parse_workflow_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="dqn regenerates seeded epsilon-greedy actions; fixed replays historical actions",
    )
...
def run_t6(records: list[dict], path: Path | None, raw_path: Path | None) -> None:
    import pandas as pd
    from T6_main import summarize_sampling
...
def generate_table(
    ...
    *,
    ...
    if table == "T3":
        ...
        run_table4(records, run_dir, selected, include_baselines, seed_map, seed)
    elif table == "T5":
        run_table5(records, run_dir, selected, include_baselines, seed_map, seed)
    else:
