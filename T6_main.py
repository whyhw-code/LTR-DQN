"""T6 sampling, aggregation and Appendix Figure C4 support."""

from __future__ import annotations

# T6 sampling and backtesting core.
...
def load_t4_full_rate(path: Path, markets: list[str]) -> dict[tuple[str, str], float]:
    """Load exact T4 ARR values for Table 6's 100% baseline rows."""
    path = path.resolve()
    if not path.is_file():
        raise FileNotFoundError(
            f"T4 results required by the T6 100% row not found: {path}. "
    ...
    if missing:
        raise ValueError(f"T4 results are missing {missing}: {path}")
    selected = frame[
    ...
    if not counts.reindex(expected, fill_value=0).eq(1).all():
        raise ValueError(
            "T6 requires exactly one T4 ARR for every market/model pair; "
            f"found:\n{counts}"
        )
...
# T6 workbook and aggregation entry point.
...
from model import CODE_DIR, RESULTS_DIR, runtime_versions, sha256, write_results
